...
- **项目与接入**：工作区下创建多个项目，每个项目配置允许的 Origin，生成公开写入专用 `collectorKey` 与一次性展示的 `queryApiKey`。
...
- **聚合存储**：事件按上海时区（`Asia/Shanghai`）分桶写入每日聚合；原始事件保留 30 天、每日聚合保留 13 个月。
...
登录（或使用真实 Google 账号）后：创建工作区 → 创建项目并填写允许的 Origin（如 `http://localhost:3000`）→ 复制“接入代码”标签中的内联片段 → 在页面中调用 `collectBrowserPulse()`，5 分钟内事件可见于项目看板。
