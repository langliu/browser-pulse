...
8. 队列消费者写入 30 天原始事件并更新每日聚合；5 分钟内控制台看板可见。
...
一个允许 Origin 上的 Chrome 页面在获得站点同意后显式调用内联 `collectBrowserPulse()`，函数返回 `accepted`，采集端点成功入队并返回 `202`，5 分钟内控制台看板的 `Chrome/<主版本>` 样本数增加 1，且相同筛选下数据明细与概览口径一致；全过程不下载额外 SDK、不创建访客 Cookie、不生成访客 ID、不保存原始 UA，也不提供对外逐事件 API。该事件通过 `collectorKey` 归属于正确项目和工作区，但不能关联到某个网站访客。
