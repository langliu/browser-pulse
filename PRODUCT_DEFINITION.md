...
- 变更摘要（相对 1.0）：正式多工作区；MVP 不含对外聚合 Query API / `queryApiKey`；采集键轮换为立即吊销旧键；允许控制台所有者查看有限「最近事件」调试视图；移除 Origin `environment` 字段
- 变更摘要（1.1.2）：取消「5 分钟」激活/可见性时限表述；改为队列消费成功后可见与清晰接入步骤，不作固定时长 SLA
...
这是一个有意的职责分离：MVP 为“一个登录账号 → 多个工作区 → 每个工作区多个项目”，事件仍归属于项目而非操作人；以后扩展成员模型时，无需迁移采集数据的归属语义。
...
- 面向由一名负责人操作 Browser Pulse 的小团队；一个 Google 登录账号可创建并管理多个工作区（默认上限 20），控制台提供工作区切换；
- 每个工作区下可创建多个项目（默认上限 50）；工作区之间数据隔离；
- 暂不提供成员邀请、加入他人工作区、角色划分或权限委派；协作成员体系留待后续版本单独设计。
...
- Google 登录；
- 一个登录账号、多个工作区及其下的多个项目（默认上限：20 工作区/账号、50 项目/工作区）；
- 工作区创建、重命名、切换与删除（删除级联清理下属数据）；
- 每个项目生成唯一、可轮换的公开采集标识 `collectorKey`；
- 控制台生成的显式调用浏览器采集代码片段，以及站内采集测试页（`/collector-test`）；
- 浏览器事件采集 API（`POST /v1/browser-events/{collectorKey}`）；
- **控制台内**浏览器版本分布、趋势、最低支持线看板与按日聚合明细（会话鉴权，非对外 API）；
- 所有者只读的「最近事件」调试视图（有限字段、有限时间窗，用于接入排障，不作访客分析）；
- 原始匿名事件 30 天、每日聚合 13 个月的保留策略；
- 项目停用、采集键轮换（旧键立即吊销）和项目数据删除。
...
- 反欺诈、实时访问拦截；
- 错误监控、性能监控或完整设备分析；
- **对外**逐事件 API、批量导出、以及任意第三方可读的原始事件接口；
- **对外**聚合查询 API 与 `queryApiKey`（后续版本可选）；
- Google 之外的登录方式；
- 成员邀请、加入他人工作区和 Owner/Admin/Viewer 角色体系；
- 计费、套餐和用量结算；
- 采集键宽限期双活（新旧键长期并存）与键命名管理 UI（后续可增强；MVP 轮换=立即吊销）。
...
| `projectId`    | Browser Pulse | 内部项目 ID；控制台路由与服务端聚合使用                   | 否                          |
| `collectorKey` | 工作区所有者  | 公开、写入专用、可轮换的项目采集标识，示例 `bpc_live_...` | 是                          |
| `visitorId`    | 不生成        | 明确禁止                                                  | 否                          |

约束：

- 首次 Google 登录只创建或绑定内部 `userId`；登录后用户显式填写工作区名称并创建首个工作区。同一账号可继续创建多个工作区（受上限约束），`workspaces.ownerUserId` 为所有者索引而非唯一约束。
- `accountId` 如需在账号设置中展示，只能作为内部客服/审计标识，不作为浏览器写入凭证。
- `collectorKey` 至少使用 128 位随机熵，全球唯一。它会公开在客户网页中，因此不是秘密，也不能提供读取权限。
- 工作区所有者可轮换 `collectorKey`：生成新活跃键并**立即吊销**旧活跃键；站点须同步更新接入代码，否则旧键返回 `401`。MVP 不要求新旧键并存宽限期。
- MVP **不发放** `queryApiKey`，不提供 Bearer 鉴权的对外聚合 HTTP API；聚合仅通过已登录控制台的 Server Functions 读取。
...
- 登录成功但尚无工作区时，控制台只展示工作区创建页；创建成功后才能进入项目页面；
- 已有工作区的账号进入所选或默认工作区，可通过选择器切换；不显示邀请入口；
- Google 登录 Cookie 仅用于 Browser Pulse 控制台，不进入客户网站的采集代码。

## 6. 主流程

1. 用户点击“使用 Google 登录”，由 Better Auth 完成 Google 回调并建立 D1 会话。
2. 首次登录只创建账号；若尚无工作区，用户填写名称创建首个工作区；已有工作区则进入所选或列表中的默认工作区，并可继续新建/切换。
3. 用户在当前工作区下创建项目，填写项目名、允许接入的 Origin，可选填写最低支持版本策略。
4. 用户打开项目详情时，系统为该项目创建（或读取）公开 `collectorKey`，并生成内联接入代码。
5. 用户复制接入代码到自有站点，在隐私告知或同意流程完成后显式调用 `collectBrowserPulse()`；也可用站内 `/collector-test` 做连通性验证（须将测试页 Origin 加入白名单）。
6. 代码片段在当前页面本地识别最小浏览器环境字段，携带 `collectorKey` 发送一次事件（同页重复调用不重复计数）。
7. 采集 Worker 校验键、Origin、正文和速率限制，成功进入队列后返回 `202`。
8. 队列消费者写入 30 天原始事件并更新每日聚合；消费成功后控制台看板可读到新样本。
9. 用户为各浏览器家族配置或调整最低支持主版本；系统即时重算低于支持线的样本占比，不改写历史原始事件。

空项目不展示空图表，而展示接入代码、Origin 配置提示和服务端最近一次可观测的拒绝原因。客户端网络失败只能由 `collectBrowserPulse()` 的返回值表达，后台不得伪称可观测。
...
### 10.1 MVP：控制台 Server Functions

MVP 的聚合读取**仅**通过已登录会话的控制台 Server Functions 完成（例如项目看板、按日明细），不经过对外 HTTP 查询 API，也不使用 `queryApiKey`。

查询语义：

- 时间窗按客户端当前时区计算；存储按 UTC 自然日分桶；
- 默认最近 30 个完整自然日；最长跨度受 13 个月聚合保留期约束；
- 可按 `osFamily`、`deviceClass` 筛选；筛选同时作用于概览、分布与趋势；
- `interval=day|week|month` 用于趋势分桶。

响应语义字段（逻辑模型，不绑定具体传输协议）：
...
`distribution` 按 `share` 降序，控制台默认展示占比前 5，并单独处理 `Unknown`。

### 10.2 后续版本（非 MVP）：对外聚合 API

若后续需要 CI/BI 集成，可另立需求提供例如：
...
届时再引入 `queryApiKey`（创建时显示一次明文、库中仅存摘要、可轮换/吊销）。MVP 验收**不依赖**该接口。

公开 HTTP 路径约定：采集使用 `/v1/...`；控制台页面与 Server Functions 不强制同一前缀。字段仅允许向后兼容新增。
...
- **支持策略**：可为浏览器家族设置整数主版本阈值（可在收到事件前预配置）；未配置家族不进入策略分母；修改阈值后按现有聚合立即重算，不改写历史事件；
- **接入设置**：Origin 白名单、项目专属代码片段（含语法高亮展示）、采集键查看/轮换、站内采集测试入口、最近拒绝原因；
- **最近事件（调试）**：所有者可查看保留期内有限条最近原始事件（最小数据字典 + `ingestId` + 采集时间），用于确认采集是否到达；界面不得称为用户/访客列表，不得提供批量导出；
- **数据明细**：按日与维度展开的聚合行，非逐访客；
- **工作区设置**：工作区名称和删除操作；控制台提供工作区切换；MVP 不展示成员、邀请或角色入口。

必须设计加载中、无数据、部分字段不可识别、策略分母为零、查询失败、超过保留期、权限不足和密钥已吊销状态。缺失样本不得被折算成零或“兼容”。同一筛选条件下，概览/分布/趋势与数据明细的聚合口径必须一致。
...
| Cloudflare 组件         | 职责                                                                                           |
| ----------------------- | ---------------------------------------------------------------------------------------------- |
| Workers + Static Assets | 承载控制台静态资源、Better Auth Google 登录与回调、会话校验、采集 API、控制台 Server Functions |
| D1                      | Better Auth 账号/会话、工作区、项目、Origin、采集键、支持策略、30 天原始事件、13 个月每日聚合  |
| Queues                  | 将采集响应与持久化解耦，批量消费、重试和削峰                                                   |
| Cron Triggers           | 每日保留期清理、会话/验证码到期清理                                                            |
| Rate Limiting binding   | 按 `collectorKey` 限制采集速率                                                                 |
...
flowchart LR
  ...
  K[控制台 Static Assets] --> L[Worker Server Functions]
  L --> G
  M[Cron Trigger] --> F
  M --> G
```
...
- `workspaces`：工作区名称、所有者 `ownerUserId`（可一对多）、创建/更新时间；MVP 不建 `memberships`；
- `projects`、`allowed_origins`：工作区下的项目及域名（Origin 精确匹配；不区分生产/测试 environment）；
- `collector_keys`：公开键材料所需字段、状态（`active`/`revoked`）和吊销时间；
- `support_policies`：浏览器家族与最低主版本；
- `raw_events`：服务端 `ingestId`、`projectId`、接收时间和最小数据字典；供重聚合、质量修复，以及控制台所有者有限调试视图；
- `daily_aggregates`：项目、UTC 自然日、筛选维度、版本维度和事件数。

MVP **不建** `query_api_keys` 表。

队列消费者必须在同一幂等边界内完成“首次写入原始事件”和“聚合计数增加”。同一 `ingestId` 重投不能重复增加聚合。项目删除或停用后，消费者必须再次检查项目状态，丢弃仍在队列中的旧消息。

### 12.3 保留与删除

- `raw_events` 保留 30 天：用于重聚合、数据质量修复，以及**仅所有者**可见的有限「最近事件」调试视图；禁止对外 API 与批量导出；
...
- 若业务要求严格境内存储，应将 D1/Queues 替换为经确认的境内数据库与消息系统，并完成数据出境评估后再上线。该变更属于部署架构变体，不改变接入代码与控制台聚合语义的产品合同。
...
- 接入路径清晰：白名单 Origin、复制代码或站内测试页、显式调用后可收到首个有效事件；
- 已接受事件经队列消费写入后，控制台看板与数据明细可读到对应聚合；
- 同一筛选条件下，看板概览/分布/趋势与数据明细的聚合口径完全一致；
- 单次采集正文不超过 1 KB；
...
一个允许 Origin 上的 Chrome 页面在获得站点同意后显式调用内联 `collectBrowserPulse()`，函数返回 `accepted`，采集端点成功入队并返回 `202`，队列消费成功后控制台看板的 `Chrome/<主版本>` 样本数增加 1，且相同筛选下数据明细与概览口径一致；全过程不下载额外 SDK、不创建访客 Cookie、不生成访客 ID、不保存原始 UA，也不提供对外逐事件 API。该事件通过 `collectorKey` 归属于正确项目和工作区，但不能关联到某个网站访客。
